...
class VideoFrameReader(object):
  ...
  def getFrameWithFrameNumber(self, frameNumber):
    ...
    while (not self.videoFrameReader.eof) or (self.currentFrameNum <= self.videoFrameReader.totalFrames):
      frame = self.videoFrameReader.getFrameWithFrameNumber(int(self.currentFrameNum))
      while not frame:
        frame = self.videoFrameReader.getFrameWithFrameNumber(int(self.currentFrameNum))
  ...
  def savePngWithFrameNumber(self, frameNumber, imageFileName):
    ...
    self.videoFrameReader.savePngWithFrameNumber(int(frameNumber), str(imageFileName))
    return True
