...
class PixelMap(object):
  ...
  def copy(self):
    ...
    newPixelMap = PixelMap(self.allCellBoundariesDict, self.neighborMap, self.scaleFactor)
...
class CellBoundaries( object ):
  def __init__( self, config ):
    ...
    staticbboxes = BoundingBoxes(
        imgDim,
        self.config.sw_xStride,
        self.config.sw_yStride, patchDim )
    self.allCellBoundariesDict = self.getCellBoundaries(staticbboxes, self.config.sw_scales)

  def getCellBoundaries(self, staticBoundingBoxes, scales):
    ...
    for scaleFactor in allScales:
      ...
      pixelCountZoomed = self.simpleZoom(pixelCount, smallestScaleRect.width, smallestScaleRect.height)
    ...
    for scaleFactor in allScales:
      ...
      pixelCountZoomed = self.simpleZoom(finalPixelCount, rect.width, rect.height)
      uniqueValues = np.unique(pixelCountZoomed)
