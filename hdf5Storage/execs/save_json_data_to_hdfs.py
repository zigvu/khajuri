...
description = \
"""
This script will save post-processed JSON data to HDF5.
This script is intended to be run in GPU1/2 machines and will send data
through RabbitMq.
It is expected that a corresponding consumer daemon is running in VM1/VM2
which pulls data out of RabbitMq and saves to HDF5.
"""
...
def process(configFileName, jsonFolder, videoId, chiaVersionId):
  ...
  for jsonFileName in jsonFolderFiles:
    logger.info("Adding: %s" % jsonFileName)
...
if __name__ == '__main__':
  if len(sys.argv) < 5:
    print 'Usage %s ' % sys.argv[0] + \
        '<config.yaml> <jsonFolder> <videoId> <chiaVersionId>'
    print description
    sys.exit(1)
