...
def displayDiff(
	...
	for cls in classes:
		print "%s,%0.3f,%s" % (cls, adiffMax[cls]['value'], adiffMax[cls]['frameNumber'])

	print "\n\nPatch Score Avg\nclass,score,frameNumber"
	for cls in classes:
		print "%s,%0.3f,%s" % (cls, adiffAvg[cls]['value'], adiffAvg[cls]['frameNumber'])

	print "\n\nPatch Score Min\nclass,score,frameNumber"
	for cls in classes:
		print "%s,%0.3f,%s" % (cls, adiffMin[cls]['value'], adiffMin[cls]['frameNumber'])

	print "\n\nLocalization Num Bbox Diff\nclass,score,frameNumber"
	for cls in classes:
		print "%s,%0.3f,%s" % (cls, aDiffNumBboxesLoc[cls]['value'], aDiffNumBboxesLoc[cls]['frameNumber'])

	print "\n\nLocalization Score Diff\nclass,score,frameNumber"
	for cls in classes:
		print "%s,%0.3f,%s" % (cls, aDiffScoreOfBboxesLoc[cls]['value'], aDiffScoreOfBboxesLoc[cls]['frameNumber'])


def main():
  if len(sys.argv) < 6:
    print 'Usage %s <config.yaml> <folder1> <old/new> <folder2> <old/new>' % sys.argv[ 0 ]
