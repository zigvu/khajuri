...
if __name__ == '__main__':
  if len(sys.argv) < 5:
    print 'Usage %s ' % sys.argv[0] + \
      '<video.file> <frame.times> <numOfFrames> <output.dir>'
    print """
    frame.times is a file with the frame times separated by spaces. 
  ...
  for second in frameTimes:
    ...
    for frame in range(
        expectedFrame - numOfFrames / 2, expectedFrame + (numOfFrames / 2) + 1):
