...
class Frame(object):
  ...
  def addLocalization(self, classId, loc):
    ...
    self.localizations[classId].append(loc)

  def localization(self, classId):
    return self.localizations[classId]
  ...
  def getLocalizationDiff(self, frame):
    ...
    for classId in self.classIds:
      ...
      if not localizationListB:
        localizationListB = []
      for loc in localizationListA:
        if loc.zDistThreshold == 0:
          filteredA.append(loc)
      for loc in localizationListB:
        if loc.zDistThreshold == 0:
          filteredB.append(loc)

      diff[classId]['bbox'] = abs(len(filteredA) - len(filteredB))
      diff[classId]['maxX'] = 0
      diff[classId]['maxY'] = 0
      diff[classId]['maxA'] = 0
      diff[classId]['maxS'] = -1
      if diff[classId]['bbox'] == 0 and len(filteredA) > 0:
        ...
        for lA in filteredA:
          ...
          if bestMetric != sys.maxint:
            diff[classId]['maxX'] = max(
                diff[classId]['maxX'], abs(lA.rect.x - match.rect.x))
            diff[classId]['maxY'] = max(
                diff[classId]['maxY'], abs(lA.rect.y - match.rect.y))
            diff[classId]['maxA'] = max(
                diff[classId]['maxA'], abs(lA.rect.A - match.rect.A))
            diff[classId]['maxS'] = max(
                diff[classId]['maxS'], abs(lA.score - match.score))
    return diff
